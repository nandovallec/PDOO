...
module ModeloQytetet
  ...
  class Jugador
    ...
    public
    attr_reader :encarcelado, :saldo, :nombre
    ...
    def actualizar_posicion(casilla)
      ...
      if casilla.soy_edificable
        ...
        if tengo_propietario
          prop_encarcelado = casilla.propietario_encarcelado()
          hipote = casilla.titulo.hipotecada
          
          if (not prop_encarcelado) and (not hipote)
        ...
        if casilla.tipo == TipoCasilla::IMPUESTO
          print "Page su IMPUESTO (200 euros)\n"
          modificar_saldo(-200)
        end
    ...
    def puedo_pagar_hipoteca(casilla)
      tengo_saldo = tengo_saldo((casilla.calcular_valor_hipoteca)*1.10)
      return tengo_saldo
    end
    ...
    def cuantas_casas_hoteles_tengo()
      ...
      @propiedades.each{
        |propiedad|
        total = total + propiedad.casilla.num_casas + propiedad.casilla.num_hoteles
      }
      return total
    end
